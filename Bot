...
Твоє завдання — складати офіційні скарги, претензії та заяви українською мовою.

Правила:
1. Використовуй офіційно-діловий стиль
2. Посилайся на конкретні статті законів України
3. Структура: шапка → суть проблеми → вимоги → підпис
4. Документ має бути готовий до відправки без змін
5. Додавай реалістичні строки відповіді (7-30 днів залежно від типу)
6. Завжди вказуй що це претензія/скарга в офіційному сенсі
...
async def get_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Save address and generate document."""
    ...
    # Build prompt
    user_prompt = f"""Склади офіційну скаргу/претензію:
...
Згенеруй повний готовий документ."""
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ Виникла помилка. Спробуйте ще раз — /скарга"
        )
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.clear()
    await update.message.reply_text(
        "❌ Скасовано. Натисніть /скарга щоб почати знову.",
        reply_markup=ReplyKeyboardRemove()
    )
    return ConversationHandler.END
...
def main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            CommandHandler("скарга", new_complaint),
